...
namespace TPC
{
    public class StateManager : MonoBehaviour
    {
        ...
        public void Init()
        {
            ...
            ignoreLayers = ~(1 << 3 | 1 << gameObject.layer | 1 <<10);
            ...
            if(GetComponent<IK.Feet.FootIK>())
            {
                GetComponent<IK.Feet.FootIK>().Init(this);
            }
        }
        ...
        bool quadratic;
        Vector3 offsetBack = new Vector3(0,0,-0.3f);
        Vector3 offsetFront = new Vector3(0,0,0.3f);
                ...
                else
                {
                    ...
                    if (numberOfHits > 2)
                    {
                        ...
                        else
                        {
                            ...
                            if (!hit.transform.GetComponentInChildren<Climbing.Manager>())
                            {
                                if (!willClimb)
                                {
            ...
            else
            {
                ...
                if (canVault && !vaulting)
                {
                    ...
                    if (Physics.Raycast(vOrigin, -Vector3.up, out vHit, Statics.vaultCheckDistance, ignoreLayers))
                    {
                        ...
                        if (Mathf.Abs(diff) < vaultFloorHeightDiffernce)
                        {
                            vaulting = true;
